...
namespace InstaSharper.API.Processors
{
    public class UserProcessor : IUserProcessor
    {
        ...
        public async Task<IResult<ICollection<InstaUser>>> GetUsersAsync(string username)
        {
            try
            {
                var userUri = UriCreator.GetUserUri(username);
                ...
                var json = await response.Content.ReadAsStringAsync();
                if (response.StatusCode != HttpStatusCode.OK)
                    return Result.UnExpectedResponse<List<InstaUser>>(response, json);
                var userInfo = JsonConvert.DeserializeObject<InstaSearchUserResponse>(json);
                if (userInfo.Users == null || userInfo.Users.Count==0)
                {
                    var errorMessage = $"Can't find this users: {username}";
                    _logger?.LogInfo(errorMessage);
                    return Result.Fail<List<InstaUser>>(errorMessage);
                }

                var returnArray = new List<InstaUser>(userInfo.Users.Count);
                foreach (var user in userInfo.Users)
                {
                    if (user.Pk < 1)
                        Result.Fail<InstaUser>("Pk is incorrect");
                    var converter = ConvertersFabric.Instance.GetUserConverter(user);
                    var convertedUser=converter.Convert();
                    returnArray.Add(convertedUser);
                }
            ...
            {
                _logger?.LogException(exception);
                return Result.Fail<List<InstaUser>>(exception.Message);
            }
        }
